...
######################################################
    ...
    def _basic_auth(self):
        ...
        if self.r.status_code == 200:
            ...
            self.sessin_expires_in = dt.fromtimestamp(response['expiresIn'])
...
######################################################
    state = fields.Selection(
        [('loginpasswd', 'Priovide Login and Password'),
    ...
    def _do_statement_import(self):
        ...
        if self.r.status_code == 200:
            ...
            self.task = 'nothing'
            # parse statement
            res = self._import_statement_data(xml_data)
            return res
        else:
            _logger.info('can not do statement. trying to recreate session')
            self._remove_session()
            return self.do_sync()
    ...
    def do_sync(self):
        ...
        build it onece again and try to do business task second time.
        '''
        self.ensure_one()
        wiz_form_act = {
            'res_id': self.id,
        ...
        try:
            if not self.session_id:
                _logger.info('no session. triyng to get one')
            ...
            if 'ROLE_P24_BUSINESS' in self.roles:
                ...
                if 'ROLE_P24_BUSINESS' in self.roles:
                    ...
                    if msg == u'Confirm authorization with OTP':
                        ...
                        _logger.info('selecet phone first')
